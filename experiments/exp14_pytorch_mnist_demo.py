"""Experiment 14: End-to-end PyTorch training with Traceprop provenance.

Demonstrates real PyTorch integration — full chain:
  raw pixels (MNIST) → preprocessing → MLP training (no BatchNorm)
  → prediction → attribution → source row identification

Model: 2-layer MLP (784 → 256 → 1), no BatchNorm, binary (digit 0 vs 1).
Shows Traceprop hooks working inside a real PyTorch training loop.
"""
...
print("=" * 60)
print("Exp 14: PyTorch MLP + Traceprop (MNIST binary, no BatchNorm)")
print("=" * 60)
...
N_TRAIN   = 6000
N_TEST    = 500
N_SUBSETS = 200       # fewer subsets — MLP retraining is heavier
PROJ_DIM  = 4096
LAMBDA    = 1e-3
EPOCHS    = 20
LR        = 1e-3
...
# ── Step 1: Load MNIST binary (0 vs 1) ───────────────────────────────────────
print("\nStep 1: Loading MNIST (digits 0 vs 1)...")
import torchvision
...
def filter_binary(dataset, n_max):
    idx = [i for i, (_, l) in enumerate(dataset) if l in (0, 1)][:n_max]
    X = torch.stack([dataset[i][0].flatten() for i in idx]).numpy()
    y = np.array([dataset[i][1] for i in idx], dtype=np.float32)
    return X.astype(np.float32), y

X_tr, y_tr = filter_binary(mnist_train, N_TRAIN)
X_te, y_te = filter_binary(mnist_test,  N_TEST)
...
# Per-sample last-layer gradients: g_i = (σ(logit_i) - y_i) * activation_i
# For the final Linear(256→1), activation = hidden layer output
...
with torch.no_grad():
    # Get hidden activations: shape (N_TRAIN, 256)
    hidden = torch.relu(model.net[0](Xt_all))   # after first Linear + ReLU
    logits = model.net[2](hidden).squeeze(1)    # final logits
    probs  = torch.sigmoid(logits)
    errors = probs - yt_all                     # (N_TRAIN,)

    # Last-layer gradient: err_i * hidden_i  (same as full gradient for linear output layer)
    G_tr = (errors[:, None] * hidden).numpy().astype(np.float32)  # (N_TRAIN, 256)
...
print(f"  Test sample 0 (digit {'0' if y_te[0]==0 else '1'}):")
for rank, idx in enumerate(top5):
    entry = store.get_entry_by_index(idx)
    print(f"    #{rank+1}: training row {idx} "
          f"(label={int(y_tr[idx])}, "
          f"source={entry.metadata.get('pixel_source','?')}, "
          f"influence={scores_0[idx]:.4f})")

print()
print("=" * 60)
print(f"{'Method':<35} {'LDS mean':>9} {'± std':>8} {'Time':>7}")
print("-" * 60)
print(f"{'TP-LL + TRAK (PyTorch MLP, no BN)':<35} {lds_mean:>9.4f} {lds_std:>8.4f} {t_attr:>6.2f}s")
print(f"{'Random baseline':<35} {rld_mean:>9.4f} {rld_std:>8.4f}    ---")
print("=" * 60)
print(f"\nMLP test accuracy: {acc:.4f}")
print(f"MLP retrain time ({N_SUBSETS} subsets): {retrain_time:.1f}s")

results = {
    "experiment": "exp14_pytorch_mnist_demo",
    "dataset": "mnist_binary_0vs1",
    ...
    "tp_ll_trak": {"lds_mean": round(lds_mean, 4), "lds_std": round(lds_std, 4), "time_s": round(t_attr, 3)},
    "random":     {"lds_mean": round(rld_mean, 4), "lds_std": round(rld_std, 4)},
    "retrain_time_s": round(retrain_time, 1),
    "notes": "Real PyTorch training loop. Per-sample last-layer gradients. No BatchNorm. Full provenance: pixel source → training row → influence score.",
}

with open("results/exp14_pytorch_mnist_demo.json", "w") as f:
    json.dump(results, f, indent=2)
print("\nSaved to results/exp14_pytorch_mnist_demo.json")
